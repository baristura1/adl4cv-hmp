...
from models.model_scatter_fixed_adj import *
...
from utils import compute_adj

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print('Using device: %s'%device)

adj = compute_adj.get_adj_human(22, 10).to(device)

model = Model(args.input_dim,args.input_n,
                           args.output_n,args.st_gcnn_dropout,args.joints_to_consider,args.n_tcnn_layers,args.tcnn_kernel_size,args.tcnn_dropout,adj).to(device)



print('total number of parameters of the network is: '+str(sum(p.numel() for p in model.parameters() if p.requires_grad)))

model_prefix='h36_3d_scatter_fixed_adj'

model_name=model_prefix+str(args.output_n)+'frames_ckpt'
      ...
      with torch.no_grad():
          ...
          for cnt,batch in enumerate(vald_loader):
              ...
              if cnt % 200 == 0:
                    ...
                    if (loss < best_val_loss) and (epoch > 40):
                        best_val_loss = loss.item()
                        # best_loss = True

                        best_model_name = model_prefix + '_best_' + str(args.output_n) + 'frames_ckpt'

                        torch.save(model.state_dict(), os.path.join(args.model_path, best_model_name))
