...
class ConvTemporalSpectral(nn.Module):
    ...
    def __init__(self,
        ...
        self.gnn_block=GNNblock(in_channels,self.time_dim,joints_dim,device,0.3,squash=False)
    ...
    def forward(self, x):
        ...
        #x = x.permute(0, 2, 1)
        #x = x.view(N, C, -1) @ self.Z
        #x = x.view(N,self.time_dim, C, self.joints_dim)
        #x = x.permute(0, 2, 1, 3)
...
class GNNblock(nn.Module):
    def __init__(self, in_feat, time_steps, feat_dim, device, do_prob, squash=True):
        ...
        self.squash=squash
        self.Scatter1= ScatteringLayer(in_feat, time_steps, feat_dim, device=self._device, num_elements=1, do_prob=self.do_prob)
        self.Scatter2 = ScatteringLayer(in_feat, time_steps, feat_dim, device=self._device, num_elements=3, do_prob=self.do_prob)
        self.SpectralAtt=SpectralAttention(in_feat, time_steps, feat_dim,9, self._device)
    ...
    def forward(self, input):
        ...
        outputs=outputs.permute(0,3, 1, 4, 2).reshape(-1,9,self.feat_dim,self.in_feat)
        att_coeffs = self.SpectralAtt(outputs)
        outputs=outputs.view((input.shape[0]*self.time_steps,9,-1)).transpose(1,2)
        outputs=torch.bmm(outputs, att_coeffs.unsqueeze(2))
        outputs=outputs.view(-1,self.time_steps,self.feat_dim,self.in_feat).permute(0,3,1,2)
        return outputs+input


class ScatteringLayer(nn.Module):

    def __init__(self, in_feat, time_steps, feat_dim, num_elements, device, do_prob):
        ...
        self.do_prob=do_prob
        self.weight1 = Parameter(torch.FloatTensor(self.time_steps, self.in_feat, self.in_feat))
        self.weight2 = Parameter(torch.FloatTensor(self.time_steps, self.in_feat, self.in_feat))
        self.weight3 = Parameter(torch.FloatTensor(self.time_steps, self.in_feat, self.in_feat))
        self.adj = Parameter(torch.FloatTensor(self.time_steps, self.feat_dim, self.feat_dim))
        N = self.feat_dim
        self.I_cached=torch.eye(N).to(self._device).unsqueeze(0).expand(self.time_steps,-1,-1)

        self.bn1 = nn.BatchNorm2d(self.num_elements * self.in_feat)
        self.bn2 = nn.BatchNorm2d(self.num_elements * self.in_feat)
        self.bn3 = nn.BatchNorm2d(self.num_elements * self.in_feat)
    ...
    def compute_wavelets_torch(self, A):
        ...
        A_norm=torch.linalg.matrix_norm(A,dim=[1,2])**2

        P = 1 / 2 * (I + torch.div(A.permute(1,2,0), A_norm).permute(2,0,1))

        H = (I - P).reshape(self.time_steps, N, N)

        H_2 = (P - torch.matrix_power(P, 2)).reshape(self.time_steps, N, N)
    ...
    def forward(self,input, bn=False, dropout=False, squash=False):
        ...
        input_reshaped=input.reshape(-1,self.in_feat,self.time_steps,self.feat_dim)
        out1 = torch.einsum('nctv,tcw->nwtv', (input_reshaped, self.weight1))
        out1 = torch.einsum('tvw, nctv->nctw', (self.adj,input_reshaped))
        if bn:
            out1 = out1.view((-1, self.num_elements* self.in_feat, self.time_steps, self.feat_dim ))
            out1=self.bn1(out1)

        out1=out1.view((-1,self.num_elements,self.in_feat, self.time_steps, self.feat_dim))

        out2 = torch.einsum('nctv,tcw->nwtv', (input_reshaped, self.weight2))
        out2 = torch.einsum('tvw, nctv->nctw', (wavelet1, input_reshaped))
        if bn:
            out2 = out2.view((-1, self.num_elements * self.in_feat, self.time_steps, self.feat_dim))
            out2 = self.bn2(out2)

        out2 = out2.view((-1, self.num_elements, self.in_feat, self.time_steps, self.feat_dim))

        out3 = torch.einsum('nctv,tcw->nwtv', (input_reshaped, self.weight3))
        out3 = torch.einsum('tvw, nctv->nctw', (wavelet2, input_reshaped))
        if bn:
            out3 = out3.view((-1, self.num_elements * self.in_feat, self.time_steps, self.feat_dim))
            out3 = self.bn3(out3)
...
class SpectralAttention(nn.Module):
    def __init__(self, in_feat, time_steps, feat_dim, num_freqs, device):
        ...
        self.channel_mlp=create_mlp(dim_input=self.in_feat*2*self.feat_dim,dim_output=self.in_feat, arch=[128])
...
class ST_GCNN_layer(nn.Module):
    ...
    def __init__(self,
        ...
        if use_wavelets:
            self.gcn = ConvTemporalSpectral(time_dim, joints_dim, in_channels)  # the convolution layer
        else:
...
class CNN_layer(
    ...
    def __init__(self,
        ...
        self.block = [nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding), nn.BatchNorm2d(out_channels), nn.Dropout(dropout, inplace=True)]
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.st_gcnns.append(ST_GCNN_layer(input_channels, 64, [1, 1], 1, input_time_frame,
                                           joints_to_consider, st_gcnn_dropout, use_wavelets=False))
