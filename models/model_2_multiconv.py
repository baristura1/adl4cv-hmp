...
class ST_GCNN_layer(nn.Module):
    ...
    def __init__(self,
        ...
        for cc1 in range(3):
            self.gcns.append(ConvTemporalGraphical(time_dim,joints_dim)) # the convolution layer
    ...
    def forward(self, x):
     #   assert A.shape[0] == self.kernel_size[1], print(A.shape[0],self.kernel_size)
        ...
        temp=list()
        for i in range(3):
            temp.append(self.gcns[i](x))
            temp[i]=self.tcns[i](temp[i])
            temp[i]=temp[i]+res
            temp[i]=self.prelu(temp[i])
