...
class ConvTemporalSpectral(nn.Module):
    ...
    def __init__(self,
        ...
        self.gnn_block=GNNblock(in_channels, time_dim,joints_dim, device,0.3,squash=False)
...
class GNNblock(nn.Module):
    def __init__(self, in_feat, time_steps, num_verts, device, do_prob, squash=True):
        super(GNNblock, self).__init__()
        self.in_feat=in_feat
        self.time_steps=time_steps
        self.num_verts=num_verts
        self.feat_dim=num_verts*time_steps
        self._device=device
        self.do_prob=do_prob
        self.squash=squash
        self.Scatter1= ScatteringLayer(in_feat, time_steps, num_verts, device=self._device, num_elements=1, do_prob=self.do_prob)
        self.Scatter2 = ScatteringLayer(in_feat, time_steps, num_verts, device=self._device, num_elements=4, do_prob=self.do_prob)
        self.SpectralAtt=SpectralAttention(in_feat, self.feat_dim,16, self._device)
    ...
    def forward(self, input):
        ...
        outputs=outputs.view((input.shape[0],16,-1)).transpose(1,2)
...
class ScatteringLayer(nn.Module):
    ...
    def __init__(self, in_feat, time_steps, num_verts, num_elements, device, do_prob):
        super(ScatteringLayer, self).__init__()
        self.in_feat=in_feat
        self.num_verts=num_verts
        self.time_steps=time_steps
        self.feat_dim=time_steps*num_verts
        self._device=device
        self.num_elements=num_elements
        self.do_prob=do_prob
        self.weight0 = Parameter(torch.FloatTensor(self.in_feat, self.in_feat))
        ...
        self.adj_space = Parameter(torch.FloatTensor(self.time_steps, self.num_verts, self.num_verts))
        self.adj_time = Parameter(torch.FloatTensor(self.num_verts, self.time_steps, self.time_steps))
    ...
    def reset(self):
        stdv = 1. / math.sqrt(self.in_feat)
        self.weight0.data.uniform_(-stdv, stdv)
        self.weight1.data.uniform_(-stdv, stdv)
        self.weight2.data.uniform_(-stdv, stdv)
        self.weight3.data.uniform_(-stdv, stdv)
        self.adj.data.uniform_(-stdv, stdv)
        stdv = 1. / math.sqrt(self.time_steps)
        self.adj_time.data.uniform_(-stdv, stdv)
        stdv = 1. / math.sqrt(self.num_verts)
        self.adj_space.data.uniform_(-stdv, stdv)
    ...
    def forward(self,input, bn=False, dropout=False, squash=False):
        ...
        out0=torch.matmul(input_reshaped, self.weight0)

        #out0=out0.transpose(1,2)

        out0=out0.view(-1,self.time_steps, self.num_verts, self.in_feat,)

        out0=out0.permute(0,3,1,2)

        out0 = torch.einsum('nctv,vtq->ncqv', (out0, self.adj_time))
        ## x=self.prelu(x)
        out0 = torch.einsum('nctv,tvw->nctw', (out0, self.adj_space))

        out0 = out0.permute(0, 2, 3, 1)

        out0=out0.reshape(-1, self.time_steps*self.num_verts, self.in_feat)

        #out0 = out0.transpose(1, 2)

        if bn:
            out0 = out0.view((-1, self.num_elements* self.feat_dim* self.in_feat))
            out0=self.bn1(out0)
        ...
        concat=torch.cat([out0,out1,out2,out3], dim=1)
        if squash:
...
class SpectralAttention(nn.Module):
    ...
    def forward(self,input):
        ...
        output=torch.cat([H_sp.unsqueeze(1).expand(-1,self.num_freqs ,-1,-1),input],dim=3)
        output=self.channel_mlp(output.reshape((output.shape[0],self.num_freqs ,-1)))
