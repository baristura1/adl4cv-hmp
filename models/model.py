...
class ConvTemporalGraphical(nn.Module):
    ...
    def __init__(self,
        ...
        self.T1=nn.Parameter(torch.FloatTensor(joints_dim , time_dim, time_dim))
        stdv = 1. / math.sqrt(self.T1.size(1))
        self.T1.data.uniform_(-stdv,stdv)

        self.T2=nn.Parameter(torch.FloatTensor(joints_dim , time_dim, time_dim))
        ...
        '''
    def forward(self, x):
        x = torch.einsum('nctv,vtq->ncqv', (x, self.T1))
        ## x=self.prelu(x)
        x = torch.einsum('nctv,tvw->nctw', (x, self.A))

        x = torch.einsum('nctv,vtq->ncqv', (x, self.T2))
