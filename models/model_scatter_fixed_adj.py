...
class ConvTemporalSpectral(nn.Module):
    ...
    def __init__(self,
                 time_dim,
                 joints_dim, in_channels, adj_matrix
                 ):
        ...
        self.gnn_block=GNNblock(in_channels, time_dim,joints_dim, device,0.3,adj_matrix, squash=False)
...
class GNNblock(nn.Module):
    def __init__(self, in_feat, time_steps, num_verts, device, do_prob, adj_matrix, squash=True):
        ...
        self.squash=squash
        self.adj_matrix=adj_matrix
        self.Scatter1= ScatteringLayer(in_feat, time_steps, num_verts, device=self._device, num_elements=1, do_prob=self.do_prob, adj_matrix=adj_matrix)
        self.Scatter2 = ScatteringLayer(in_feat, time_steps, num_verts, device=self._device, num_elements=3, do_prob=self.do_prob, adj_matrix=adj_matrix)
        self.SpectralAtt=SpectralAttention(in_feat, self.feat_dim,9, self._device)
...
class ScatteringLayer(nn.Module):
    ...
    def __init__(self, in_feat, time_steps, num_verts, num_elements, device, do_prob, adj_matrix):
    ...
    def forward(self,input, bn=False, dropout=False, squash=False):
        ...
        wavelet1, wavelet2=self.compute_wavelets_torch(self.adj_matrix)
...
class ST_GCNN_layer(nn.Module):
    ...
    def __init__(self,
        ...
        self.gcn = ConvTemporalSpectral(time_dim, joints_dim, in_channels, adj_matrix)  # the convolution layer
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.st_gcnns.append(ST_GCNN_layer(input_channels, 64, [1, 1], 1, input_time_frame,
                                           joints_to_consider, st_gcnn_dropout, adj_matrix))
