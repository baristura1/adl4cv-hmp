...
def main(opt):
    ...
    net_pred = AttModule.AttModule(in_n=opt.input_n, kernel=kernel_size, hidden_size=d_model,
                                   out_n=opt.output_n, feat_dim=in_features)
    #net_pred.cuda()
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        if is_train <= 1:  # if is validation or train simply output the overall mean error
            #print("p3d_h36[:, in_n:in_n + out_n]", p3d_h36[:, in_n:in_n + out_n].shape)
